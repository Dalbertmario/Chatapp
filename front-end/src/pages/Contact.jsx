...
function handelUserClick(el){
  ...
  return (
    <div className=" flex flex-col  gap-3 sm:w-[100%] xl:w-[30%] lg:w-[50%] md:w-[50%] p-2">
      ...
      {search ? data?.map((el)=>(<button onClick={()=>handelUserClick(el)} className="bg-white  hover:bg-slate-200 transition-all font-bold p-3 rounded-l text-start" key={el._id}>{el?.name}</button>)) : contactData?.map((el)=><button onClick={()=>handelUserClick(el)} className="bg-white font-bold outline-1 outline-slate-500 p-3 rounded-l text-start flex flex-row justify-between" key={el?.name}>{el?.name} <span className={`${el?.notification === 0 ?  ' ' : 'bg-green-400 text-sm px-2 text-slate-700 flex items-center rounded-3xl' }`}>{el?.notification === 0  ? '' : el?.notification}</span></button>)}
    </div>
  )
}
